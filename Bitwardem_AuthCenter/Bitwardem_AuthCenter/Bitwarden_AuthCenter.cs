...
public class Bitwarden_AuthCenter : IDisposable
{
    ...
    public Item CreateLogin(string orgId, string collectionId, string itemname, string username, string password, string uri, string notes = "some notes here")
    {
        ...
        item.login.uris = new List<Models.Uri>();
        item.login.uris.Add(new Models.Uri { uri = uri });

        item.fields = new List<Field>();
        item.fields.Add(new Field { name = "Security Question", value = "Bitwarden Rules", type = (int)Models.FieldType.Text });
